...
import com.savegift.dto.EmailDto;
import com.savegift.dto.SmsDto;
...
public class UserRepository {
	...
	public boolean login(HashMap<String, Object> requestMap) {
		...
		User user = mybatis.selectOne("LoginMapper.duplicationid", requestMap);
		if(user != null) { // 아이디는 있음
			...
			if(userPasswordDB != null) {// DB와 사용자의 입력 값을 비교함
				if(userPasswordDB.equals(userPasswordInput)) {
					if(index != null) { // 회원정보 수정
						...
						Notification notification = mybatis.selectOne("NotificationMapper.userDevice", device_id);
						if(notification != null) { // 이미 insert 이력이 있음.
	...
	public boolean duplicationid(String user_id) {
		boolean result = false;
		User user = mybatis.selectOne("LoginMapper.duplicationid", user_id);

		if(user != null) { //SELECT => TRUE
			result = true;
		}
		return result;
	}
	...
	public boolean checkSocial(String user_id) {
		boolean result = false;
		User user = mybatis.selectOne("LoginMapper.checkSocialId", user_id);
	...
	public User findEmail(String user_id) {
		//where절 id가 널이 아닐때 -> 그 id로 비밀번호 불러옴 -> input 비밀번호랑 db 비밀번호랑 비교
		User user = mybatis.selectOne("LoginMapper.findemail", user_id);
	...
	public User findPhone(String phone_number) {
		//where절 id가 널이 아닐때 -> 그 id로 비밀번호 불러옴 -> input 비밀번호랑 db 비밀번호랑 비교
		User user = mybatis.selectOne("LoginMapper.findphone", phone_number);
	...
	public int addFriend(String user_id, String friend, String name) {
		...
		if(result == 1) {
			...
			Friend friendVO = mybatis.selectOne("LoginMapper.addFriend", user_id); // 본인이 이미 추가한 사용자
			...
			Friend friendVO2 = mybatis.selectOne("LoginMapper.addFriendOpponent", friendOrigin);// 상대방 update 및 insert
			if(friendVO2 != null) {
				//update
	...
	public int deleteFriend(String user_id, String friend) {
		...
		// 로그인 사용자에서 해당 친구 삭제
		Friend friendVO = mybatis.selectOne("LoginMapper.addFriend", map);
		...
		List<Friend> list = mybatis.selectList("LoginMapper.addFriend", map);
	...
	public String statusFriend(String user_id, String friend, String name) {
		...
		Friend friendVO1 = null;
		Friend friendVO2 = null;
		Friend friendVO3 = null;
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("user_id", user_id);
		map.put("name", name);
		map.put("friend", friend);
	...
	public Friend getFriend(String user_id) {
		Friend friend = null;
		friend = mybatis.selectOne("LoginMapper.addFriend", user_id);
...
//			mybatis.delete("LoginMapper.secessionUserDevice", user_id);
	...
	public User userInfo(String user_id) {
		User user = mybatis.selectOne("LoginMapper.userInfo", user_id);
		return user;
	}
	...
	public boolean checkEmailInfo(HashMap<String, Object> requestMap) {
		User user = mybatis.selectOne("LoginMapper.checkEmailInfo", requestMap);
	...
	public boolean checkEmail(HashMap<String, Object> requestMap) {
		EmailDto emailDto = mybatis.selectOne("LoginMapper.checkEmail", requestMap);
	...
	public boolean checkNamePhone(HashMap<String, Object> requestMap) {
		List<User> list = mybatis.selectList("LoginMapper.checkNamePhone", requestMap);
	...
	public boolean certNumberDB(String user_id, String cert_number) {
		...
		EmailDto emailDto = mybatis.selectOne("LoginMapper.emailCheckSelect", map);
	...
	public String sendSMS(String phone_number, String certNum, String device_id, HashMap<String, String> params) {
		...
		SmsDto smsDto = mybatis.selectOne("LoginMapper.smsSelect", map);

		if(smsDto != null) { // 당일에 2번 이상 시도
			count = smsDto.getCount();
			if(count == 3) { // 3번째 시도
	...
	public boolean smsCheck(HashMap<String, Object> requestMap) {
		SmsDto smsDto = mybatis.selectOne("LoginMapper.smsCheck", requestMap);
